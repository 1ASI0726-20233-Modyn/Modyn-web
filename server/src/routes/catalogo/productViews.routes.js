...
router.post("/", async (req, res) => {
    try {
        const body = req.body;
        const ultimo = await ProductView.findOne({}, {}, { sort: { VIEW_id: -1 } });
        const nuevoId = ultimo ? ultimo.VIEW_id + 1 : 1;
        const respuesta = await ProductView.create({ ...body, VIEW_id: nuevoId });
        res.send(respuesta);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
