...
router.post("/", async (req, res) => {
    try {
        const body = req.body;
        const ultimo = await ProductLike.findOne({}, {}, { sort: { PLK_id: -1 } });
        const nuevoId = ultimo ? ultimo.PLK_id + 1 : 1;
        const respuesta = await ProductLike.create({ ...body, PLK_id: nuevoId });
        res.send(respuesta);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
