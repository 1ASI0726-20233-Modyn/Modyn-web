...
router.post("/", async (req, res) => {
    try {
        const body = req.body;
        const ultimo = await Recommendation.findOne({}, {}, { sort: { REC_id: -1 } });
        const nuevoId = ultimo ? ultimo.REC_id + 1 : 1;
        const respuesta = await Recommendation.create({ ...body, REC_id: nuevoId });
        res.send(respuesta);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
