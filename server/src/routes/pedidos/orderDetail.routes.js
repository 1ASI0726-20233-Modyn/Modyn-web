...
router.post("/", async (req, res) => {
    try {
        const body = req.body;
        const ultimo = await OrderDetail.findOne({}, {}, { sort: { DET_id: -1 } });
        const nuevoId = ultimo ? ultimo.DET_id + 1 : 1;
        const respuesta = await OrderDetail.create({ ...body, DET_id: nuevoId });
        res.send(respuesta);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
